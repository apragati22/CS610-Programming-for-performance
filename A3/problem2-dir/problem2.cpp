...
#define N (1 << 24)  // 16M elements for better benchmarking
...
// AVX2 version - processes 8 integers at a time (256 bits)
// Uses Kogge-Stone parallel prefix sum algorithm within each 256-bit register
int avx2_version(const int* __restrict__ source, int* __restrict__ dest) {
  source = static_cast<int*>(__builtin_assume_aligned(source, ALIGN));
  dest = static_cast<int*>(__builtin_assume_aligned(dest, ALIGN));

  __m256i offset = _mm256_setzero_si256();
  const int stride = AVX2_WIDTH_BITS / (sizeof(int) * CHAR_BIT);  // 8 elements
  
  for (int i = 0; i < N; i += stride) {
    // Load 8 integers: [h, g, f, e | d, c, b, a]
    // AVX2 operates on two 128-bit lanes: upper [h,g,f,e] | lower [d,c,b,a]
    __m256i x = _mm256_load_si256((__m256i*)&source[i]);
    
    // Step 1: Shift by 1 element and add
    // _mm256_slli_si256 shifts within each 128-bit lane independently
    __m256i shifted = _mm256_slli_si256(x, 4);  // [g,f,e,0 | c,b,a,0]
    
    // Handle cross-lane data movement: move element 3 (d) to position 4
    __m256i permuted = _mm256_permute2x128_si256(x, x, 0x08);  // [0,0,0,0 | h,g,f,e]
    shifted = _mm256_blend_epi32(shifted, permuted, 0x10);      // [g,f,e,d | c,b,a,0]
    
    x = _mm256_add_epi32(x, shifted);
    // Result: [g+h, f+g, e+f, d+e | c+d, b+c, a+b, a]
    
    // Step 2: Shift by 2 elements and add
    shifted = _mm256_slli_si256(x, 8);  // Shift within lanes
    permuted = _mm256_permute2x128_si256(x, x, 0x08);
    shifted = _mm256_blend_epi32(shifted, permuted, 0x30);  // Blend positions 4 and 5
    
    x = _mm256_add_epi32(x, shifted);
    // Result: [e..h, d..g, c..f, b..e | c..d, b..c, a..b, a]
    
    // Step 3: Shift by 4 elements (entire lower lane to upper)
    shifted = _mm256_permute2x128_si256(x, x, 0x08);
    x = _mm256_add_epi32(x, shifted);
    // Result: [a..h, a..g, a..f, a..e | a..d, a..c, a..b, a]
    
    // Add cumulative offset from previous iterations
    x = _mm256_add_epi32(x, offset);
    
    // Store result
    _mm256_store_si256((__m256i*)&dest[i], x);
    
    // Extract last element (sum of all 8) and broadcast for next iteration
    __m128i high = _mm256_extracti128_si256(x, 1);  // Extract upper 128 bits
    __m128i last = _mm_shuffle_epi32(high, _MM_SHUFFLE(3, 3, 3, 3));
    offset = _mm256_broadcastd_epi32(last);  // Broadcast to all 8 elements
  }
...
__attribute__((optimize("no-tree-vectorize"))) int main() {
  ...
  cout << "Array size: " << N << " elements (" << (N * sizeof(int) / (1024.0 * 1024.0)) << " MB)" << endl;
